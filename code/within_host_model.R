...
deriv <- function(t, state, parms) {
  with(as.list(c(state, parms)), {
    ...
    # dP1 <- (alpha1 * delta1 * P1 * R) - (beta1 * I1 * P1) - (beta2 * I2 * P1)  # Complete neutrality scenario.
    # dP2 <- (alpha2 * delta2 * P2 * R) - (beta2 * I2 * P2) - (beta1 * I1 * P2)  # Complete neutrality scenario.

    dI1 <- (epsilon * beta1 * I1 * P1) + (sigma1 * M1 * P1) + (tau * M2 * P1) - (gamma * I1)
    dI2 <- (epsilon * beta2 * I2 * P2) + (sigma2 * M2 * P2) + (tau * M1 * P2) - (gamma * I2)
    
    # dI1 <- (epsilon * beta1 * I1 * P1) + (epsilon * beta2 * I2 * P1) + (sigma1 * M1 * P1) + (sigma2 * M2 * P1) + (tau * M2 * P1) + (tau * M1 * P1) - (gamma * I1) # Complete neutrality scenario.
    # dI2 <- (epsilon * beta2 * I2 * P2) + (epsilon * beta1 * I1 * P2) + (sigma2 * M2 * P2) + (sigma1 * M1 * P2) + (tau * M1 * P2) + (tau * M2 * P2) - (gamma * I2) # Complete neutrality scenario.
    
    dM1 <- (rho * I1) + (rho * I2) - (mu * M1)
    dM2 <- (rho * I2) + (rho * I1) - (mu * M2)
